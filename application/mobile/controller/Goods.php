...
/**
 * ============================================================================
 * $Author: IT宇宙人 2015-08-10  edit 邓小明 2017.05.04
 */
...
class Goods extends MobileBase {
    ...
    public function _initialize() {      
        ...
        //微信js分享   goodsInfo
        $goodsInfo='http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
        $this->assign('goodsUrl',$goodsInfo);
        //商品名字
        $goodsId=preg_replace('/\D/s', '', $_SERVER["QUERY_STRING"]);
        $goodsName=M('goods')->where('goods_id',$goodsId)->field('goods_name')->find();
        ...
        $url='http://'.$_SERVER['HTTP_HOST'].U('Mobile/ThirdLogin/myShare').'?shareUid='.$this->user_id.'&goodsUrl='.$goodsInfo;
        ...
        $this->assign('goodsName',$goodsName['goods_name']);
    ...
    public function goodsInfo(){ 
        ...
        $goods_id = I("get.id");
        //区分促销商品与普通商品
        if(I('get.prom_goods')==1){
            $goods=M('flash_sale')->query("select fs.*,gs.goods_name,gs.market_price,gs.shop_price,gs.store_count,gs.brand_id,gs.goods_content from __PREFIX__flash_sale fs left join __PREFIX__goods gs on gs.goods_id=fs.goods_id where fs.goods_id={$goods_id} limit 0,1");
            //session('prom_goods',I('get.prom_goods'));
            $goods=$goods[0];
            $this->assign('prom_goods',I('get.prom_goods'));
        }else{
    ...
    public function collect_goods(){
        $res=array();
        $data=array('goods_id'=>I('goods_id'),'user_id'=> cookie('user_id'),'add_time'=>time());
        $resCheck=M('goods_collect')->where('goods_id='.I('goods_id'))->select();
        if($resCheck){
            M('goods_collect')->where('goods_id='.I('goods_id'))->delete();
            $res['msg']=-1;
            echo json_encode($res);
        }else{
            $insertId=M('goods_collect')->add($data);   
            if($insertId){
                $res['msg']=1;
                echo json_encode($res);
            }else{
