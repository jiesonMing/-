    ...
    public function _initialize() {
        ...
        if(strstr($_SERVER['HTTP_USER_AGENT'],'MicroMessenger') && empty($_SESSION['openid'])){
            ...
            if(is_array($this->weixin_config) && $this->weixin_config['wait_access'] == 1){
                ...
                // if($data['msg']==1){
                //     session('wxUserInfo',$data['result']);
                //     $this->redirect('Mobile/ThirdLogin/setMobile');//验证手机号
                //     exit;
                // }    
                //是否有绑定pc账号
                $bind=M('users')->where("oauth='weixin' and user_id=".$data['result']['user_id'])->field('bindId')->find();
                if($data['status'] == 1){
                    //有绑定账号
                    if($bind['bindId']){
                        $user=M('users')->where("user_id=".$bind['bindId'])->find();
                        $data['result']=$user;
                    }
...
//            $tpshop_config=M('config')->where("inc_type='shop_info' and name='store_title'")->find();       
    ...
    public function GetOpenid()
    {
        ...
        if (!isset($_GET['code'])){
            ...
            $data['oauth'] = 'weixin';
            $data['pay_points'] = tpCache('basic.reg_integral'); // 会员注册赠送积分
            $data['salt']=rand(000000,999999);
            $data['parentUser']= !empty(I('shareUid'))?I('shareUid'):I('id');
            if(!empty($data2['unionid'])){
