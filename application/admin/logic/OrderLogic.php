...
class OrderLogic extends Model
{
    ...
    public function getOrderList($condition,$order='',$start=0,$page_size=20){
        if($order==''){
            $order ='field(o.payOrderStatus,4,2,1,0,3),o.payOrderInfo desc,field(o.cusOrderStatus,4,2,1,0,3),o.cusOrderInfo desc,field(o.ciqOrderStatus,4,2,1,0,3),field(o.isSync,4,0,3,1,2),o.add_time desc';
        }
        $sql = "select o.*,u.userType,u.nickname,w.warehouse_name,w.warehouse_type from __PREFIX__order as o left join __PREFIX__users as u on u.user_id=o.user_id left join __PREFIX__warehouse as w on w.warehouse_id=o.wareId where 0=0 {$condition} order by {$order} limit {$start},{$page_size}";
        $res = DB::query($sql);
        return $res;
    }
    /*
    ...
    public function get_spec_goods($goods_id_arr){
        $orderUserLevel= empty(session('orderUserLevel'))?0:session('orderUserLevel');
    	if(!is_array($goods_id_arr)) return false;
    		foreach($goods_id_arr as $key => $val)
    		{
    			...
    			$goods = M('goods')->where("goods_id = $key")->find();
                        ...
                        }else if($orderUserLevel==1){
                            $arr['member_goods_price'] = $goods['thirdMemberPrice'];
                        }else if($orderUserLevel==2){
                            $arr['member_goods_price'] = $goods['secondMemberPrice'];
                        }else if($orderUserLevel==3){
                            $arr['member_goods_price'] = $goods['firstMemberPrice'];
                        }
    ...
    public function getOrderButton($order){
        ...
        $wareId = $order['wareId'];//1-保税仓，2-普通仓
        $btn = array();
    ...
    public function deliveryHandle($data){
        //公司的名称与英文代码
        $deliveryInfo=explode(',', $data['shipping_name']);
        ...
        $data['shipping_name'] = $deliveryInfo[0];
        ...
        $updata['express_code']  = $deliveryInfo[1];
        //确认发货后的状态
        $updata['outStoreStatus']= 1;
        $updata['outStoreTime']= date('Y-m-d H:i:s',time());  
		...
		$s = $this->orderActionLog($order['order_id'],'delivery','商家发货');//操作日志
		return $s && $r;
    }
