...
class User extends Base {
    ...
    public function ajaxindex(){
        ...
        $where='';
        if($nickname){
            $where.="username like '%{$nickname}%' or nickname like '%{$nickname}%' or mobile like '{$nickname}' or email like '{$nickname}'";
        }
       ...
       /* foreach($condition as $key=>$val) {
        ...
        if($where){
            $sql="select u.*,ur.role_name from __PREFIX__users as u left join __PREFIX__user_role as ur on ur.role_level=u.myLevel where u.username like '%{$nickname}%' or u.nickname like '%{$nickname}%' or u.mobile like '{$nickname}' or u.email like '{$nickname}' order by ".$sort_order." limit {$Page->firstRow},{$Page->listRows}";
            $userList=$model->query($sql);
            //$userList = $model->alias('u')->join('__PREFIX__user_role as ur on ur.role_level=u.myLevel')->where($where)->order($sort_order)->limit($Page->firstRow.','.$Page->listRows)->select();
        }else{
            //$userList = $model->order($sort_order)->limit($Page->firstRow.','.$Page->listRows)->select();
            $sql="select u.*,ur.role_name from __PREFIX__users as u left join __PREFIX__user_role as ur on ur.role_level=u.myLevel order by ".$sort_order." limit {$Page->firstRow},{$Page->listRows}";
            $userList=$model->query($sql);
        }
    ...
    public function detail(){
        ...
        $userRole=M('User_role')->field('role_name,role_level')->order('role_id desc')->select();
    ...
    public function myLevelList(){
        $list=M('User_role')->field('role_id,role_name,role_level')->order('role_id desc')->select();
        $this->assign('list',$list);
        return $this->fetch();
    }
    
    public function changeMyLevel(){
        $id=I('id/d');
        if(IS_POST){
            $role_name=I('role_name/s');
            if(!$role_name) errorMsg (400, '会员等级名称不能为空');
            $list=M('User_role')->field('role_id')->where("role_name='{$role_name}'")->find();
            if($list) errorMsg (400, '该会员等级名称已经存在');
            
            $data['role_name']=$role_name;
            $ret=M('User_role')->where('role_id='.$id)->save($data);
            
            if($ret){
                errorMsg(200, '修改会员等级名称成功');
            }elseif($ret==0){
                errorMsg(400, '没做任何修改');
            }else{
                errorMsg(400, '网络繁忙，请稍候再试');
            }
        }else{
            $list=M('User_role')->field('role_id,role_name')->where('role_id='.$id)->find();
            $this->assign('list',$list);
    ...
    public function editWithdrawals()
    {
        ...
        if (IS_POST) {
            ...
            if ($data['status'] == 1 && $withdrawals['status'] != 1) {
                if ($user['withdrawaling'] < $withdrawals['money']) {
            ...
            //扣除账户提现中的金额记录
            M('action')->add(array('username'=>'用户提现','tabName'=>'users','tabField'=>'withdrawaling='.$withdrawals['money'],'notes'=>'扣除账户提现中的金额记录'));
    ...
    public function withdrawals_update(){
        ...
        if($status == 1){
            ...
            foreach($withdrawals as $v){
               $sql="update __PREFIX__users set withdrawaling=withdrawaling-{$v['money']} where user_id={$v['user_id']}";
               M('users')->execute($sql);
            }
            $r = M('withdrawals')->where('id','in', $id)->save(array('status'=>$status,'check_time'=>time()));           
        }else if($status == -1){
            $this->ajaxReturn(array('status'=>1,'msg'=>"ces2"),'JSON');exit;
            $r = M('withdrawals')->where('id','in', $id)->save(array('status'=>$status,'refuse_time'=>time()));
        }else if($status == 2){
            $this->ajaxReturn(array('status'=>1,'msg'=>"ces3"),'JSON');exit;
            ...
            $this->ajaxReturn(array('status'=>1,'msg'=>"ces4"),'JSON');exit;
            $r = M('withdrawals')->where('id in ('.implode(',', $id).')')->delete();
        }else{
            $this->ajaxReturn(array('status'=>1,'msg'=>"ces5"),'JSON');exit;
    ...
    public function child(){
        $id = I('id');
        $model = M('users');
        $count = $model->where('parentUser='.$id)->count();
        $Page  = new AjaxPage($count,10);
        $userList = $model->where('parentUser='.$id)->limit($Page->firstRow.','.$Page->listRows)->select();
        $show  = $Page->show();
        $this->assign('userList',$userList);
        $this->assign('level',M('user_level')->getField('level_id,level_name'));
        $this->assign('page',$show);// 赋值分页输出
    ...
    public function distribution(){
        $model = M('account_log');
        $where='1 ';
        if(I('parentUsername')||I('nickname'))
            $where="(concat(pu.nickname,pu.username,pu.user_id) like '%".I('parentUsername','nickname','strip_tags')."%' or concat(u.nickname,u.username) like '%".I('nickname','parentUsername','strip_tags')."%')";
        if(I('start_time'))
            $where.=" and (al.change_time>=".strtotime(I('start_time'))." and al.change_time<=".strtotime(I('end_time')).")";
        $sql="select count(*) as count from __PREFIX__account_log al left join __PREFIX__order o on o.order_sn=al.order_sn left join __PREFIX__users u on u.user_id=o.user_id left join __PREFIX__users pu on pu.user_id=u.parentUser where al.desc='会员分销提成' and {$where}";
        ...
        //分页个数
        if(I('pageNum'))
            session('pageNum',I('pageNum'));
        if(session('pageNum'))
            $pageNumMoren=session('pageNum');
        $pageNum=$pageNumMoren?$pageNumMoren:10;    
        $Page  = new AjaxPage($count[0]['count'],$pageNum);
        $sql="select al.log_id as id,al.desc,al.user_money,al.order_sn,al.change_time,u.nickname,u.parentUser,pu.nickname as parentUsername from __PREFIX__account_log al left join __PREFIX__order o on o.order_sn=al.order_sn left join __PREFIX__users u on u.user_id=o.user_id left join __PREFIX__users pu on pu.user_id=u.parentUser where al.desc='会员分销提成' and {$where} order by log_id desc limit {$Page->firstRow},{$Page->listRows}";
        $list=$model->query($sql);
        $show  = $Page->show();
        $this->assign('list',$list);
        $this->assign('show',$show);// 赋值分页输出
        $this->assign('condition',I(''));
    ...
    public function exportData(){
        if(IS_GET){
            $sql="select al.log_id as id,al.desc,al.user_money,al.order_sn,al.change_time,u.nickname,u.parentUser,pu.nickname as parentUsername from __PREFIX__account_log al left join __PREFIX__order o on o.order_sn=al.order_sn left join __PREFIX__users u on u.user_id=o.user_id left join __PREFIX__users pu on pu.user_id=u.parentUser where al.log_id in (".I('ids').")";
            $resData=M('account_log')->query($sql);

            $strTable ='<table width="1000px" border="1">';
            $strTable .= '<tr>';
            $strTable .= '<td style="text-align:center;font-size:12px;width:80px;height:40px;font-weright:bold;">获利用户id</td>';
            $strTable .= '<td style="text-align:center;font-size:12px;width:140px;height:40px;font-weright:bold;">获利用户名称</td>';
            $strTable .= '<td style="text-align:center;font-size:12px;width:80px;height:40px;font-weright:bold;">获利金额</td>';
            $strTable .= '<td style="text-align:center;font-size:12px;width:120px;height:40px;font-weright:bold;">订单编号</td>';
            $strTable .= '<td style="text-align:center;font-size:12px;width:140px;height:40px;font-weright:bold;">下单人</td>';
            $strTable .= '<td style="text-align:center;font-size:12px;width:120px;height:40px;font-weright:bold;">获利分销时间</td>';
            $strTable .= '<td style="text-align:center;font-size:12px;width:100px;height:40px;font-weright:bold;">描述</td>';
            $strTable .= '</tr>';
            if(is_array($resData)){
                foreach($resData as $k=>$val){
                    $strTable .= '<tr>';
                    $strTable .= '<td style="text-align:center;height:30px;font-size:12px;">&nbsp;'.$val['parentUser'].'</td>';
                    $strTable .= '<td style="text-align:center;height:30px;font-size:12px;">'.$val['parentUsername'].' </td>';
                    $strTable .= '<td style="text-align:center;height:30px;font-size:12px;">'.$val['user_money'].'</td>';
                    $strTable .= '<td style="text-align:center;height:30px;font-size:12px;">'.$val['order_sn'].'</td>';
                    $strTable .= '<td style="text-align:center;height:30px;font-size:12px;">'.$val['nickname'].'</td>';
                    $strTable .= '<td style="text-align:center;height:30px;font-size:12px;">'.date("Y-m-d H:s:i",$val['change_time']).'</td>';
                    $strTable .= '<td style="text-align:center;height:30px;font-size:12px;">'.$val['desc'].'</td>';
                    $strTable .= '</tr>';
                }
            }
            $strTable .='</table>';
            unset($resData);
            downloadExcel($strTable,'会员分销记录');
            exit();
        }else{
            errorMsg(200,'导出成功',I('ids'));
        }
    }
}