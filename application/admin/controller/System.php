...
class System extends Base{
	...
	{          
		/*配置列表*/
		$group_list = array('shop_info'=>'网站信息','basic'=>'基本设置','sms'=>'短信设置','shopping'=>'购物流程设置','smtp'=>'邮件设置','water'=>'水印设置','distribut'=>'分销设置','system'=>'系统设置');		
		...
		}elseif($inc_type == 'system'){//系统配置
                    $config_settings=M('settings')->find();
                    $config_shop=M('shop')->find();
                    $config_pay_shop=M('pay_shop')->find();
                    $this->assign('config_setting',$config_settings);
                    $this->assign('config_shop',$config_shop);
                    $this->assign('config_pay_shop',$config_pay_shop);
                }
	...
	public function handle()
	{
		...
		//unset($param['__hash__']);
                if($param['inc_type']=='system'){
                    if(I('post.settings_id')){
                        M('settings')->where('id='.I('post.settings_id'))->save($param);
                    }elseif(I('post.shop_id')){
                        M('shop')->where('id='.I('post.shop_id'))->save($param);
                    }elseif(I('post.pay_shop_id')){
                        $data = array(
                            'code'=>$param['code'],
                            'payCusCode'=>$param['payCusCode'],
                            'payCompanyName'=>$param['payCompanyName'],
                            'payurl'=>$param['payurl'],
                            'payid'=>$param['payid'],
                            'paytoken'=>$param['paytoken'],
                            'intelpayid'=>$param['intelpayid'],
                            'intelpaytoken'=>$param['intelpaytoken'],
                            'payType'=>$param['payType'],
                            'customspaytype'=>$param['customspaytype'],
                            'customsNo'=>$param['customsNo'],
                            'customsName'=>$param['customsName'],
                            'customsCode'=>$param['customsCode'],
                            'ciqCode'=>$param['ciqCode'],
                            'ciqNo'=>$param['ciqNo'],
                            'ciqBNo'=>$param['ciqBNo'],
                            'shopDomain'=>$param['shopDomain'],
                            );
                        // dump($data);exit;
                        M('settings')->where('id=1')->save($data);
                        M('pay_shop')->where('id='.I('post.pay_shop_id'))->save($param);
                    }
                }else{
