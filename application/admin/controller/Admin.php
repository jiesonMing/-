...
//            }
    ...
    public function address_keyword(){
        $list = M('address_keyword')->select();
        $this->assign('list',$list);
        return $this->fetch();
    }
    ...
    public function edit_keyword(){
        $id = I('get.id/d',0);
        if($id){
            $info = D('address_keyword')->where("id", $id)->find();
            $this->assign('info',$info);
        }
        if (I('post.')) {
            $data = array(
                'keyword' => myTrim(I('post.keyword')),
                );
            if(I('post.id/d')){
                $r = M('address_keyword')->where('id', I('post.id/d'))->save($data);
            }else{
                $r = M('address_keyword')->add($data);
            }
            if($r){
                $this->success("操作成功",U('Admin/Admin/address_keyword'));
            }else{
                $this->error("操作失败",U('Admin/Admin/address_keyword'));
            }
        }
        return $this->fetch();
    }
    ...
    public function del_keyword(){
        $id = I('post.id/d');
        if($id){
            $r = M('address_keyword')->where('id',$id)->delete();
            exit(json_encode(1));
        }
