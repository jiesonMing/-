...
//        $this->assign('function',$plugin_list['function']);
        ...
        $this->assign('type',I('type')?I('type'):'shipping');       
        return $this->fetch();
    }
...
//        $row = $model->where($condition)->save($update);
    ...
    public function add_shipping()
    {
        ...
        if(!empty(I('shipping_id'))){
            $ret=M('shipping')->where('shipping_id',I('shipping_id'))->save(I('post.'));
            if($ret){
                $this->success("修改成功", U("Plugin/index"));
            }else{
                $this->error('修改失败');
            }
        }else{
            $code = strtolower(I('shipping_code'));// 编码
            $name = I('shipping_name'); // 物流名字
            $desc = I('shipping_desc', '');// 插件描述
            
            if (!preg_match("/[a-zA-Z]{2,20}/", $code))
                $this->error("物流编码必须 2-20位小写字母组成");
            $shipping = M('shipping')->where("shipping_code", $code)->find();
            $shipping && $this->error("编码 $code 已存在");
            
            $sql = "insert into __PREFIX__shipping(shipping_code,shipping_name,shipping_desc,enabled) values('{$code}','{$name}','{$desc}',1)";
            $ret = M('shipping')->execute($sql);
            if($ret){
    ...
    public function editShipping(){
        if(IS_GET){
            $shipping=M('shipping')->where('shipping_id',I('shipping_id'))->find();
            $this->assign('shipping',$shipping);
            return $this->fetch('_shipping');
            exit;
        }
     ...
     * 删除物流
     */
    public function del_shipping($shipping_id){
        ...
        $ret = M('shipping')->where("shipping_id",$shipping_id)->save($data);
        if($ret){
            errorMsg(200,'删除成功！');
        }else{
            errorMsg(400,'删除失败！');
    ...
    //物流改变状态
    public function chengStatus($shipping_id){
        $data = array(
            'enabled'=>1
            );
        $ret = M('shipping')->where("shipping_id",$shipping_id)->save($data);
