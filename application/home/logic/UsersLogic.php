...
class UsersLogic extends Model
{
    ...
    public function login($username,$password){
        ...
        //$user = M('users')->where("oauth='' and (username='{$username}' or email='{$username}' or mobile='{$username}')")->find();
        //edit in 09.25 只要是微信账号绑定了手机号的，也是可以登录
        //$user = M('users')->where("(oauth='' or (oauth !='' and email !='' or mobile !='')) and (username='{$username}' or email='{$username}' or mobile='{$username}')")->find();
       $sql="select * from __PREFIX__users where (oauth='' or (oauth!='' and email!='' or mobile!='')) and (username='{$username}' or email='{$username}' or mobile='{$username}')";
    ...
    public function app_login($username,$password){
        ...
        if(!$username || !$password)
           $result= array('status'=>0,'msg'=>'请填写账号或密码');
        $user = M('users')->where("mobile|email|username","=",$username)->find();
        if(!$user){
           $result = array('status'=>-1,'msg'=>'账号不存在!');
    ...
    public function thirdLogin($data=array()){
        ...
        if(!$user){
            ...
            $map['pay_points']=tpCache('basic.reg_integral');//注册赠送积分
            //测试在新增用户前跳转到绑定手机号页面-start
            //return array('status'=>1,'msg'=>1,'result'=>$map);
            //exit;         
            //测试在新增用户前跳转到绑定手机号页面-end  
            ...
            $user['token'] = md5(time().mt_rand(1,999999999));
            M('users')->where("user_id", $user['user_id'])->save(array('token'=>$user['token'],'head_pic'=>$data['head_pic'],'nickname'=>$data['nickname'],'sex'=>$data['sex'],'openid'=>$data['openid'],'unionid'=>$data['unionid'],'last_login'=>time()));
        }
    ...
    public function reg($username,$password,$password2){
        ...
        if(session ('parentUser'))
            $uid=urldecode(base64_decode(session('parentUser')));
            $uid=preg_replace('/\D/s', '', $uid);
            $map['parentUser'] = $uid;
        $salt=rand(100000,999999);
        $map['password'] = md5($password.$salt);
        $map['salt']=$salt;
        $map['reg_time'] = time();
        $map['first_leader'] = cookie('first_leader'); // 推荐人id
        // 如果找到他老爸还要找他爷爷他祖父等
    ...
    public function get_info($user_id)
    {
     ...
     public function get_info2($user_id){
         if(!$user_id > 0)
    ...
    public function get_order_goods($order_id){
        $sql = "SELECT og.* FROM __PREFIX__order_goods og LEFT JOIN __PREFIX__goods g ON g.goods_id = og.goods_id WHERE og.prom_type in (0,1) and  order_id = :order_id";
        $bind['order_id'] = $order_id;
        $goods_list = DB::query($sql,$bind);
        $orderSql = "select add_time,pay_status,deleted,order_status from __PREFIX__order where order_id={$order_id}";
        $orderArr = DB::query($orderSql);
        ...
        $user = session('user');
        $data = $orderAction = array();
        if($changeOrder){//检查商品信息是否修改过
            if($orderArr[0]['pay_status']!=1 && $orderArr[0]['pay_status']!=2 && ($orderArr[0]['deleted']==0) && ($orderArr[0]['order_status']==0)){//未支付，订单作废
                $data['order_status'] = 5;
                //order_action
                $orderAction['order_id'] = $order_id;
                $orderAction['action_user'] = $user['user_id']?$user['user_id']:0;
                $orderAction['action_note'] = '未支付，订单作废';
                $orderAction['log_time'] = time();
                $orderAction['status_desc'] = '系统作废订单，因商品信息改变而且未支付';
            }
        }

        if((($orderArr[0]['add_time']+(60*60*24))<time()) && ($orderArr[0]['pay_status']!=1) && ($orderArr[0]['deleted']==0) && ($orderArr[0]['order_status']==0)){//超时未支付，订单超时处理
            $data['order_status'] = 7;

            $orderAction['order_id'] = $order_id;
            $orderAction['action_user'] = $user['user_id']?$user['user_id']:0;
            $orderAction['action_note'] = '超时未支付，订单超时处理';
            $orderAction['log_time'] = time();
            $orderAction['status_desc'] = '系统作废订单，因商品信息改变而且超时未支付';
        }
        M('order')->where('order_id',$order_id)->save($data);
        if($orderAction) M('order_action')->add($orderAction);
        
        $sql2 = "SELECT og.* FROM __PREFIX__order_goods og LEFT JOIN __PREFIX__goods g ON g.goods_id = og.goods_id WHERE order_id = :order_id";
        $goods_list2 = DB::query($sql,$bind);

        $return['status'] = 1;
        $return['msg'] = '';
        $return['result'] = $goods_list2;
        return $return;
    }
...
//        $type = I('get.type',0);
     ...
     * 获取用户等级一级下级用户
     */
    public function getSubUser($userId){
        $count=M('users')->where('parentUser='.$userId)->count();
        $Page = new Page($count,10);
        $sql="select u.user_id,u.nickname,u.mobile,u.email,u.last_login,u.reg_time,ur.role_name from __PREFIX__users as u left join __PREFIX__user_role as ur on ur.role_level=u.myLevel where u.parentUser={$userId} order by u.myLevel desc,u.user_id DESC limit {$Page->firstRow},{$Page->listRows}";
        $users=M('users')->query($sql);
        //$users=M('users')->alias('u')->join('__PREFIX__user_role as ur on ur.role_level=u.myLevel')->field('u.user_id,u.nickname,u.mobile,u.email,u.last_login,u.reg_time,u.myLevel,ur.role_name')->where('u.parentUser=:parentUser')->bind(['u.parentUser'=>$userId])->order('u.myLevel desc,u.user_id DESC')->limit($Page->firstRow.','.$Page->listRows)->select();
        $return['status'] = 1;
        $return['msg'] = '';
        $return['result'] = $users;
        $return['show'] = $Page->show();
    ...
    public function get_comment($user_id,$status=2){
        if($status == 1){
            ...
            $sql = "select c.*,g.*,(select order_sn from  __PREFIX__order where order_status in(1,2,4) and  order_id = c.order_id ) as order_sn  from  __PREFIX__comment as c inner join __PREFIX__order_goods as g on c.goods_id = g.goods_id and c.order_id = g.order_id where c.user_id = $user_id order by c.add_time desc LIMIT {$page->firstRow},{$page->listRows}";
        }else{        	
        	$countsql = " select count(1) as comment_count from __PREFIX__order_goods as og
        	left join __PREFIX__order as o on o.order_id = og.order_id where o.order_status in (1,2,4) and o.user_id = $user_id";
        	$where = '';
        	if($status == 0){
    ...
    public function add_comment($add){
        ...
        if($row)
        {
            ...
            //评论完成 结算分销提成
            calculateTicheng(4,$add['order_id']);
    ...
    public function add_address($user_id,$address_id=0,$data){
        ...
        $post['buyerIdNumber'] = strtoupper($post['buyerIdNumber']);
    ...
    public function password($user_id,$old_password,$new_password,$confirm_password,$is_update=true){
        ...
        //验证原密码
        $oldpass=md5($old_password.$user['salt']);
        if($is_update && ($user['password'] != '' && $oldpass != $user['password']))
            return array('status'=>-1,'msg'=>'原密码验证失败','result'=>'');
        $newpass=md5($new_password.$user['salt']);
        $row = M('users')->where("user_id", $user_id)->save(array('password'=>$newpass));
        if(!$row)
    ...
    public function cancel_order($user_id,$order_id){
        ...
        $row = M('order')->where(array('order_id'=>$order_id,'user_id'=>$user_id))->save(array('order_status'=>3,'pay_time'=>0));
    ...
    public function check_validate_code1($code, $sender, $type ='email', $session_id=0 ,$scene = -1){
    ...
    //设置手机号，密码，用于登录 $mobile,$password,$userInfoData
    public function setMobile($mobile,$password){
        Session::start();
        $wxUserInfo=session('wxUserInfo');
        if(!$wxUserInfo)
            return array('status'=>-1,'msg'=>'登录已超时，请关闭页面重新进入！','result'=>'');
        if($password==''){
            //该手机号已注册了PC账号，把该微信号绑定到此手机号
            $resUser=M('users')->where('mobile',$mobile)->find();
            $wxUserInfo=array_merge($wxUserInfo,$resUser);
        }
        $salt=rand(0,999999);
        $wxUserInfo['mobile']=$mobile;
        $wxUserInfo['salt']=$salt;
        $wxUserInfo['password']=md5($password.$salt);
        M('users')->startTrans();
        if($wxUserInfo['user_id']){
            //更新
            $res=M('users')->where('user_id',$wxUserInfo['user_id'])->save($wxUserInfo);
            $user_id=$wxUserInfo['user_id'];
        }else{
            //新增
            $res=M('users')->add($wxUserInfo);
        ...
        if($res){
            ...
            return array('status'=>1,'msg'=>'登录成功','result'=>'');                      
        }else{
            //添加失败
            M('users')->rollback();
            return array('status'=>-1,'msg'=>'登录失败','result'=>'');         
        }
    }
