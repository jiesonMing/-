...
class Index extends Base {
    ...
    public function index(){
        ...
        $sql = "select a.goods_name,a.goods_id,a.shop_price,a.firstMemberPrice,a.secondMemberPrice,a.thirdMemberPrice,a.market_price,a.cat_id,b.parent_id_path,b.name from ".C('database.prefix')."goods as a left join ";
        $sql .= C('database.prefix')."goods_category as b on a.cat_id=b.id where a.is_hot=1 and a.is_on_sale=1 order by a.sort asc,a.goods_id desc";//二级分类下热卖商品       
        ...
        if(session('user')){
            $users=session('user');
            $this->myLevel=$users['myLevel'];
        }
        $config = tpCache('shop_info');
        $province = M('region')->where(array('id'=>$config['province']))->find();
        $this->assign('province',$province['name']);
        $city = M('region')->where(array('id'=>$config['city']))->find();
        $this->assign('city',$city['name']);
        $area = M('region')->where(array('id'=>$config['district']))->find();
