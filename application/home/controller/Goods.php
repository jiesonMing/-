...
class Goods extends Base {
    ...
    */ 
    public function goodsInfo(){ 
        ...
        //print_r($goods['goods_content']);exit;   
        $this->assign('goods',$goods);
        $this->assign('goodsCatId',$goods['cat_id']);
        $this->assign('goods_name',addslashes($goods['goods_name']));
        $this->assign('brand_id',$goods['brand_id']);
    ...
    public function goodsList(){ 
        ...
        if($isNew){
            $filter_goods_id = M('goods')->where(['is_on_sale'=>1,'is_new'=>$isNew])->cache(true)->getField("goods_id",true);
        }else{
    ...
    public function search()
    {
       //C('URL_MODEL',0);
        ...
        if($count > 0)
        {
            ...
            $filter_goods_id2 = get_arr_column($goods_list, 'goods_id');
            ihttp://i2.265g.com/images/201710/201710100134121274.jpgf($filter_goods_id2)
            $goods_images = M('goods_images')->where("goods_id", "in",implode(',', $filter_goods_id2))->select();
        }    
        
        if($this->mylevel==1){
            $this->assign('priceSort','thirdMemberPrice');
        }elseif($this->mylevel==2){
            $this->assign('priceSort','secondMemberPrice');
        }elseif($this->mylevel==3){
            $this->assign('priceSort','firstMemberPrice');            
        }else{
            $this->assign('priceSort','shop_price');            
        }
    ...
    public function ajaxComment(){
        ...
        $where = ['c.is_show'=>1,'c.goods_id'=>$goods_id,'c.parent_id'=>0];
        if($commentType==5){
            $where['img'] = ['<>',''];
        }else{
        	$typeArr = array('1'=>'0,1,2,3,4,5','2'=>'4,5','3'=>'3','4'=>'0,1,2');
            $where['ceil((c.deliver_rank + c.goods_rank + c.service_rank) / 3)'] = ['in',$typeArr[$commentType]];
        }
        $count = M('Comment')->alias('c')->join('__USERS__ u','u.user_id = c.user_id','LEFT')->where($where)->count();                
        
        $page = new AjaxPage($count,2);
        $show = $page->show();
       
        $list = M('Comment')->alias('c')->join('__USERS__ u','u.user_id = c.user_id','LEFT')->where($where)->field('c.*')->order("add_time desc")->limit($page->firstRow.','.$page->listRows)->select();
        ...
        foreach($list as $k => $v){
            ...
            if(!empty($v['username'])){
                $len = strlen($v['username']);
                $hideNum = floor($len*(2/3));
                $list[$k]['username'] = substr_replace($v['username'], '***', 0, $hideNum);
            }else{
