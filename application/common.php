...
function getCatGrandson ($cat_id)
{
    ...
    // 把整张表找出来
    $GLOBALS['category_id_arr'] = M('goods_category')->cache(true,TPSHOP_CACHE_TIME)->getField('id,parent_id');
    // 先把所有儿子找出来
    $son_id_arr = M('goods_category')->where("parent_id", $cat_id)->cache(true,TPSHOP_CACHE_TIME)->getField('id',true);
...
function getOrderGoodNum($goods_id)
{
    return  M("Goods")->where("goods_id", $goods_id)->getField('store_count');
}
...
function accountLog($user_id, $user_money = 0,$pay_points = 0, $desc = '',$distribut_money = 0){
    ...
    if($desc=='会员在线充值'){
        ...
        $sql = "UPDATE __PREFIX__users SET withdrawaling = withdrawaling + $user_money," .
            " pay_points = pay_points + $pay_points, distribut_money = distribut_money + $distribut_money WHERE user_id = $user_id";
    }
    if( DB::execute($sql)){
...
function orderStatusDesc($order_id = 0, $order = array())
{
    ...
    {
        if($order['pay_status'] == 0 && $order['order_status'] == 0 && $order['deleted']!=1)
            return 'WAITPAY'; //'待支付',
        if($order['pay_status'] == 2 && $order['order_status'] == 0 && $order['deleted']!=1)
            return 'PAYING';//‘支付中’
        if($order['pay_status'] == 4 && $order['order_status'] == 0 && $order['deleted']!=1)
            return 'FAILEDPAY';//‘支付失败’
        if($order['pay_status'] == 1 &&  in_array($order['order_status'],array(0,1)) && $order['shipping_status'] != 1 && $order['deleted']!=1)
            return 'WAITSEND'; //'待发货',
    }
    ...
    if($order['order_status'] == 3 || $order['deleted']==1)
        return 'CANCEL'; //'已取消',
    ...
    if($order['order_status'] == 6)
        return 'BACKMONEY'; //'已退款',
    if($order['order_status'] == 7)
        return 'ORDERTIMEOUT'; //'已超时',
    return 'OTHER';
}
...
function orderBtn($order_id = 0, $order = array())
{
    ...
    else
    {
        ...
        if( ($order['pay_status'] == 2 || $order['pay_status']==3) && $order['order_status'] == 0 and $order['deleted']!=1) // 待支付
        {
            $btn_arr['repay_btn'] = 1; // 重新支付
            $btn_arr['cancel_btn'] = 1; // 取消按钮
        }
...
function update_pay_status($order_sn,$ext=array())
{
    ...
    function confirm_order($id,$user_id = 0){
        ...
        $data['order_status'] = 2; // 已收货   关于后台重新加载后，状态改变的修改       
        ...
        //分销提成
        //order_commission($order); 改为收货完成之后 15天之后或者其他时间
        //分销设置
     ...
     *分销提成1.0
     */
    function order_commission($order){
        set_time_limit(0);
        //找出当前订单的用户有是否有上级，有得话就把上级的会员等级找出,没有就退出
        $current_level=M('users')->where('user_id',$order['user_id'])->cache(true)->find();//当前的订单等级
        if($current_level['parentUser']!=0){
            $parent_level=M('users')->where('user_id',$current_level['parentUser'])->cache(true)->find();//当前订单有上级时的等级
            //查找当前订单下所有的商品
            $order_goods = M('order_goods')->where("order_id",$order['order_id'])->cache(true)->select();
            $amount=0;//提成金额
            foreach ($order_goods as $v){
                $goods=M('goods')->where("goods_id",$v['goods_id'])->cache(true)->find();
                //是否是同一级别
            ...
            //把提成金额做记录
           $data=array('user_id'=>$current_level['parentUser'],'user_money'=>$amount,'change_time'=>time(),'desc'=>'会员分销提成','order_sn'=>$order['order_sn'],'order_id'=>$order['order_id']);
           M('account_log')->add($data);
           $sql="update __PREFIX__users set user_money=user_money+".$amount.' where user_id='.$current_level['parentUser'];
           M('users')->execute($sql);
        }
    }
    //计算分销提成2.0
    function calculateTicheng($order_status,$order_id){    
        set_time_limit(0);
        $model=M('order');
        $sql="select o.*,u.myLevel,u.parentUser from __PREFIX__order o left join __PREFIX__users u on u.user_id=o.user_id where o.isDistribut!=1 and o.order_id=".$order_id;
        $orderData=$model->query($sql);
        //return $amount=$orderData[0]['parentUser'];
        $ret=0;
        if($orderData[0]['parentUser']!='' && $orderData[0]['yujTicheng']!=0){
            if($order_status==4){
                //收货评论完成的，为上级添加提成，把提成金额做记录
                $data=array('user_id'=>$orderData[0]['parentUser'],'user_money'=>$orderData[0]['yujTicheng'],'change_time'=>time(),'desc'=>'会员分销提成','order_sn'=>$orderData[0]['order_sn'],'order_id'=>$order_id);
                $res1=M('account_log')->add($data);
                $sql="update __PREFIX__users set user_money=user_money+".$orderData[0]['yujTicheng'].' where user_id='.$orderData[0]['parentUser'];
                $model->startTrans();
                $res2=$model->execute($sql);
                if($res1 && $res2){                   
                    //把已经提成过的设置已提成
                    $sql="update __PREFIX__order set isDistribut=1 where order_id=".$order_id;
                    $model->execute($sql);
                    $model->commit();
                    $ret=1;
                }else{
            ...
            M('order')->where('order_id',$order_id)->save(array('yujTicheng'=>$amount));
        }
        return $ret;
    }
/**
...
function calculate_price($user_id = 0, $order_goods, $shipping = 0,$shipping_price= 0, $province = 0, $city = 0, $district = 0, $pay_points = 0, $user_money=0, $coupon_id = 0,$couponCode= 0)
{
    ...
    foreach ($order_goods as $key => $val) {
        ...
        $order_goods[$key]['store_count'] = getOrderGoodNum($val['goods_id']); // 最多可购买的库存数量
        ...
        $taxTotal += round($val['member_goods_price']*$val['goods_num']*$val['tax'],2);//商品税
    }
    // 优惠券处理操作
    $coupon_price=0;
   ...
   /* if ($shipping_price == 0) {
    ...
    $order_amount = $goods_price + $shipping_price - $coupon_price + $taxTotal; // 应付金额 = 商品价格 + 物流费 - 优惠券 + 税
...
//读取excel文件的内容
    ...
    //支付完成，商品的库存和已售的改变
    function changeGoodsNum($orderNo){
        $model=M('order');
        $sql="select ogs.goods_id,ogs.goods_num,gs.store_count from __PREFIX__order o left join __PREFIX__order_goods ogs on ogs.order_id=o.order_id left join __PREFIX__goods gs on gs.goods_id=ogs.goods_id where o.order_sn='{$orderNo}'";
        $goodsNum=$model->query($sql);
        $sqlArr=array();
        //库存，已售数量
        foreach ($goodsNum as $key => $value) {
            if($value['store_count']>0 && $value['store_count']>$value['goods_num']){//库存数量大于0并且还有大于商品的购买数量
                $sqlArr[]="update __PREFIX__goods set sales_sum=sales_sum+{$value['goods_num']},store_count=store_count-{$value['goods_num']} where goods_id={$value['goods_id']}";
            }            
        }
        if(!empty($sqlArr)){
